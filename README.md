...
HW_2/
  ...
  archive/
    old_notebooks/                      # intermediate notebooks, not needed for final review
    old_outputs/                        # intermediate outputs, not needed for final review
    planning/                           # initial implementation plan
  .venv_hw_2/                           # local virtual environment, ignored by git
```
...
They are stored in [`data/`](data/) and are part of the submitted materials because the assignment requires empirical data to be attached.

The prepared parquet cache is stored in [`cache/db1b_market_combined.parquet`](cache/db1b_market_combined.parquet). It is generated by [`notebooks/01_db1b_market_eda.ipynb`](notebooks/01_db1b_market_eda.ipynb). The cache is useful for fast reruns but can be regenerated from the raw CSV files.
...
It does not save exploratory plots, candidate-route files, or other intermediate outputs.
...
The final notebook does not save intermediate CSV/PNG outputs. Its tables and plots are displayed directly inside the notebook.
